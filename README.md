...
A full-stack geospatial environmental analytics platform for managing
carbon and biodiversity projects, visualizing geographical sites, and
understanding environmental performance over time.
...
Core flow: Create Project → Draw Site → Store Polygon → Match
Environmental Data → Analyze → Visualize Historical Changes.
...
Different datasets are used because each answers a different
...
The actual dataset columns are inspected during import rather than
assumed.
...
biodiversity_observations
        ↓
Spatial query against site polygon
        ↓
Species inside the site
...
Limitation: observations are not necessarily a complete inventory of
every species present.

2. NASA MODIS MOD13Q1 NDVI

Product: MOD13Q1.061 --- MODIS/Terra Vegetation Indices 16-Day L3
...
How has vegetation condition changed over time?
...
Important: NDVI is a vegetation index. It is not direct carbon
stock.
...
Used for forest and carbon-related environmental context, depending
on the actual fields available.
...
The application preserves the meaning and limitations given by the
source. Measured, modeled, estimated, and derived values should be
...
Global Species          What biodiversity       Species, observations,
Abundance and Diversity observations exist      abundance/biomass
here?

NASA MODIS MOD13Q1      How has vegetation      NDVI and vegetation
condition changed?      trends
...
Forest / Carbon
     ↓
Site Environmental Analytics
     ↓
Historical Trends

Geospatial Data Flow

The main feature is connecting an administrator-created site boundary
with environmental data.

1. Draw site

The administrator draws a polygon using Mapbox.
...
4. Match biodiversity

Coordinates from the biodiversity dataset become PostGIS points.

Conceptually:

SELECT *
FROM biodiversity_observations
WHERE ST_Within(location, site_geometry);

5. Calculate analytics

FastAPI performs the required spatial filtering and aggregation.
...
The application is designed to show change over time, not only the
current state.

For a selected site, the dashboard can show available information such
as:
...
If a dataset has no data for a year:
...
Database credentials remain on the backend and are not exposed to the
browser.
...
NASA MODIS: https://modis.gsfc.nasa.gov/data/dataprod/mod13.php
...
Biodiversity observations depend on source sampling and geographic
coverage.
...
Spatial aggregation depends on the resolution and structure of each
dataset.

Darukaa.Earth

Geospatial intelligence for environmental monitoring, biodiversity
analysis, and data-driven conservation.
